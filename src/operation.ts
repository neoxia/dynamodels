...
export abstract class Operation<T> {
  ...
  constructor(documentClient: DocumentClient, params: DocumentClient.QueryInput | DocumentClient.ScanInput) {
   ...
   * @param filterConditions The filter condition as an object where keys are fields and value the targeted value
   * for this field. In this case an equal 'EQ' operation is done. Value can also be a call to an operator
  ...
  /**
   * Apply filters to the scan/query operation
   * @param filterConditions The filter condition as the result of chainable attr(), or(), and(), not() and operator
   * helpers calls.
   * @returns The filtered query/scan
   */
   ...
   * @param isConsistent (Optional) whether or not the operation should be read-consistent. true if ommited.
  ...
  public projection(
    ...
    const expression = fields.map((f) => {
      if (typeof f === 'string') {
        attributes[`#${f}`] = f;
        return `#${f}`;
      } else if ((f as { list: string; index: number }).list !== undefined) {
        const l = f as { list: string; index: number };
        attributes[`#${l.list}`] = l.list;
        return `#${l.list}[${l.index}]`;
      } else if ((f as { map: string; key: string }).map !== undefined) {
